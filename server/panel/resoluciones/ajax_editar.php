<?php
    ...
    if($editURL){
        ...
        if($resultadoUP){
            $query = "UPDATE `resoluciones` SET `nombre`='$nombres', `tipo`='$tipo', `fecha`=now(), `url_archivo`='$uploadedFile' WHERE `idresolucion`='$id';";
            if(is_file($file)){
        ...
        $query = "UPDATE `resoluciones` SET `nombre`='$nombres', `tipo`='$tipo', `fecha`=now() WHERE `idresolucion`='$id';";
        $resultadoUP = true;
    }
