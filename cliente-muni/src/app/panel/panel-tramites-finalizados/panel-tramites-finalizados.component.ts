...
@Component({
  selector: 'app-panel-tramites-finalizados',
  templateUrl: './panel-tramites-finalizados.component.html',
  styleUrls: ['./panel-tramites-finalizados.component.css']
})
export class PanelTramitesFinalizadosComponent implements OnInit {
  ...
  listar(page: number){
    ...
    try{
      this.service.getDataFinalizados(this.pages[page-1]).subscribe(
        res => {
          this.ready = true;
          this.tramites = res
        },
      ...
      console.log(error);
      this.pageActive -= 1;
      this.service.getDataFinalizados(this.pages[page-2]).subscribe(
        res => {
          this.ready = true;
          this.tramites = res
        },
  ...
  filtrar(index: string){
    ...
    if(index==""){
      ...
      this.ready = false;
      this.service.getDataFilter(index, "2").subscribe(
        res =>{
          this.ready = true;
          this.tramites = res;
        },
