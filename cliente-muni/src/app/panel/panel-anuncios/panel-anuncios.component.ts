...
export class PanelAnunciosComponent implements OnInit {
  ...
  constructor(private service: PanelAnunciosService, private toastr: ToastrService) { 
    this.ready = false;
  ...
  submit(getForm){ 
    ...
    if(this.anuncio.id){
      if(this.photoSelected){
        this.btn = false;
        this.service.putData(this.anuncio, this.file).subscribe(
          (res) => {
            this.btn = true;
      ...
      if(this.photoSelected){
        this.btn = false;
        this.service.postData(this.anuncio, this.file).subscribe(
          (res) => {
            this.btn = true;
  ...
  eliminar(getElement: any){
    if(getElement.id){
      this.btn = true;
      this.anuncio = getElement;
    }else{
      this.btn = false;
      this.service.deleteData(this.anuncio).subscribe(
        res => {
          this.btn = true;
  ...
  edit(getElement: Anuncio){
    this.anuncio = Object.assign({},getElement);
    this.photoSelected = this.URI+"upload/anuncios/"+this.anuncio.url_img;   
  }
