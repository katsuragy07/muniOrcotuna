...
@Component({
  selector: 'app-panel-tramites-pendientes',
  templateUrl: './panel-tramites-pendientes.component.html',
  styleUrls: ['./panel-tramites-pendientes.component.css']
})

export class PanelTramitesPendientesComponent implements OnInit {
  ...
  closeFormDerivar(getForm){
    $("#modal-derivar").modal('hide');
    getForm.resetForm();
  }
  ...
  paginar(){
    this.ready = false;
    this.service.getPages(this.pageSize, "pendientes").subscribe(
      res => {
        //console.log(res)
        this.pages = res;
  ...
  listar(page: number){
    ...
    try{
      this.service.getDataPendientes(this.pages[page-1]).subscribe(
        res => {
          this.ready = true;
          this.tramites = res
        },
      ...
      console.log(error);
      this.pageActive -= 1;
      this.service.getDataPendientes(this.pages[page-2]).subscribe(
        res => {
          this.ready = true;
          this.tramites = res
        },
  ...
  finalizar(getElement: any){
    if(getElement.id){
      ...
      this.btn = false;
      this.service.finalizar(this.tramite).subscribe(
        res => {
          this.btn = true;
          this.toastr.success('Tramite finalizado con exito.','Aviso');
          this.paginar();
          this.closeConfirm();
  ...
  derivar(getForm){
    this.btn = false;
      this.service.derivar(this.tramite, this.areaDest).subscribe(
        res => {
          this.btn = true;
          this.toastr.success('Tramite derivado con exito.','Aviso');
          this.paginar();
          this.closeFormDerivar(getForm);
        },
        err => {
          console.log(err);
          this.btn = true;
