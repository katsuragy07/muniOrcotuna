...
@Component({
  selector: 'app-panel-tramites-rechazados',
  templateUrl: './panel-tramites-rechazados.component.html',
  styleUrls: ['./panel-tramites-rechazados.component.css']
})

export class PanelTramitesRechazadosComponent implements OnInit {
  ...
  paginar(){
    this.ready = false;
    this.service.getPages(this.pageSize, "rechazados").subscribe(
      res => {
        //console.log(res)
        this.pages = res;
  ...
  listar(page: number){
    ...
    try{
      this.service.getDataRechazados(this.pages[page-1]).subscribe(
        res => {
          this.ready = true;
          this.tramites = res
        },
      ...
      console.log(error);
      this.pageActive -= 1;
      this.service.getDataRechazados(this.pages[page-2]).subscribe(
        res => {
          this.ready = true;
          this.tramites = res
        },
  ...
  filtrar(index: string){
    ...
    if(index==""){
      ...
      this.ready = false;
      this.service.getDataFilter(index, "3").subscribe(
        res =>{
          this.ready = true;
          this.tramites = res;
        },
